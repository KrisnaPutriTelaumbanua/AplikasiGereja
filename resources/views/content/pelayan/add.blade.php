...
@section('judul', 'Tambah Data Pelayan')

@section('content')
    <form method="post" action="{{ url('pelayan/insert') }}">
        @csrf
        <div class="row">
            <div class="col-12">
                <div class="card">
                            ...
                            <select name="id_departemen"
                                ...
                                @foreach ($departemens as $departemen)
                                    <option value="{{ $departemen->id_departemen }}"
                                        {{ old('id_departemen') == $departemen->id_departemen ? 'selected' : '' }}>
                            ...
                            <select name="id_subdepartemen"
                                ...
                                @foreach ($subdepartemens as $subdepartemen)
                                    <option value="{{ $subdepartemen->id_subdepartemen }}"
                                        {{ old('id_subdepartemen') == $subdepartemen->id_subdepartemen ? 'selected' : '' }}>
